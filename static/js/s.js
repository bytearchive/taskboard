    ...
    ACTIONS = {
        ...
        c: function ($card) {
            ...
            if (color) {
                ...
                color = COLORS[++color < COLORS.length ? color : 0]; // 6 is COLORS.length
...
function markCardChanged(card) {
    card.data(P_STATE, S_CHANGED);
    var d = new Date()
    var date_str = "" + d.getFullYear() + "-" + (d.getMonth()+1) + "-" + d.getDate() + " " + 
                d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds();
    card.data(P_LAST_UPDATE, date_str);
}
        ...
        .dlg(CARD, MOUSEDOWN, function (event) {
            ...
            if (!$body.hC(EDIT) && !$body.hC(MARK)) { // .hasClass
                ...
                $card.clone()          // clone deck card and add it to the board
                    .pick().aC(DRAG)
                    .css($card.offset())
                    .data(P_UUID, Math.uuid())      // when adding a new 'todo', give it a uuid
                    .saveText(tip())
                    .to($board)
            ...
            if (!$(event.target).is(TAG + "," + ACTION)) {
                ...
                if ($text[0][CONTENTEDITABLE] != TRUE) {
                    ...
                    $document.bind(KEYDOWN, function (keyEvent) { // bind hot-keys listener
                            ...
                            if (keyEvent.ctrlKey && keyEvent.which == 13) { // CTRL + ENTER
                                closeEdit();
                                $card.to($board);
                            }
            ...
            if (!$body.hC(EDIT) && !$(mouseDownEvent.target).is(TAG + "," + ACTION)) { // .hasClass
                ...
                $document
                    .bind(MOUSEMOVE, function (moveEvent) {
                        ...
                        $card.to($board); // move the card to the top
                    });
