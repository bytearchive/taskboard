...
def _add_new_tips():
    ...
    added_tips = db(query).select(db.meta_tip.id)
    unexpried_tips = db(db.meta_tip.expired_date > datetime.now()).select()
    fresh_tips = filter(lambda t: not t in added_tips, unexpried_tips)
